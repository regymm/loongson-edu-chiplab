...
void shell14(void)
{
	...
	if(err == 0){
        printf("fireye D1 PASS!\n");
		*((int *)LED_RG1_ADDR) = 1;  
		*((int *)LED_RG0_ADDR) = 1;  
		*((int *)LED_ADDR)     = 0xffff;  
	}else{
        printf("fireye D1 ERROE!!!\n");
		*((int *)LED_RG1_ADDR) = 1;  
		*((int *)LED_RG0_ADDR) = 2;  
		*((int *)LED_ADDR)     = 0;
	}
    ...
    *((volatile unsigned *)CONFREG_CR1) = total_count;  
	printf("fireye D1: Total Count(SoC count) = 0x%x\n", total_count);
	printf("fireye D1: Total Count(CPU count) = 0x%x\n", total_count_my);
